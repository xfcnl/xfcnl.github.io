...
(function () {
  ...
  var dynamicUrl = (window.BA_CONFIG && window.BA_CONFIG.dynamicUrl) || 'https://raw.gh.1s.fan/xfcnl/get-bilibili-dynamic-for-ci/main/dynamic.json';
  if (sidebarDyn) {
    fetch(dynamicUrl)
      ...
      .then(function(data) {
        ...
        items.forEach(function(d) {
          ...
          html += '<div class="sidebar-dyn-pill" style="' +
            ...
            '<span style="font-size:.7rem;color:var(--ba-text-muted)">' + d.time + '</span>' +
            '</div>';
        });
  ...
  (function () {
    ...
    var apiUrl = 'https://api.github.com/repos/xfcnl/ba-assets/contents/img/' + folder;
    var rawBase = 'https://raw.gh.1s.fan/xfcnl/ba-assets/refs/heads/main/img/' + folder + '/';
    ...
    try {
      ...
      if (cached && cached.time && Date.now() - cached.time < ttl && cached.names && cached.names.length) {
    ...
    fetch(apiUrl)
      ...
      .then(function (data) {
        ...
        var names = data.filter(function (f) { return f.type === 'file'; }).map(function (f) { return f.name; });
        if (!names.length) return;
        try {
          localStorage.setItem(cacheKey, JSON.stringify({ time: Date.now(), names: names }));
        } catch (e) {}
        applyBg(names);
      })
      .catch(function () {});
  ...
  (function() {
    ...
    btn.style.cssText = 'position:fixed;bottom:2rem;right:2rem;z-index:999;' +
